...
async function secondCard(deckId) {
  let data = await fetch(`https://deckofcardsapi.com/api/deck/${deckId}/draw/?count=1`)
  .then((response) => response.json())
  .then((data) => {
    return data
  })

  return console.log(Your card is the {data.cards[0].value} of {data.cards[0].suit} with deck ID of {data.deck_id});
}

  const handleClick = (e) => {
    e.preventDefault();
    let card = secondCard(deckIdPerm);
